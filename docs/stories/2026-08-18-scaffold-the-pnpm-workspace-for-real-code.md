...
As a contributor, I want the pnpm workspace scaffolded per ADR-0002 (private
...
2. All three workspace members exist with manifests and entry stubs honoring
   ADR-0002 boundaries: `packages/dsh-next-app` declares no `workspace:*`
...
4. `packages/dsh-api` contains the pinned dsh version constant
   ...
   (ADR-0002, ADR-0002); the README supported-dsh-version line is updated to
   match (ADR-0004).
...
- Workspace layout and dependency boundaries per ADR-0002; Next.js App
