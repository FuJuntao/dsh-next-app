...
6. `/bookkeeping` is user-invoked by command, `disable-model-invocation: true`, ships as `.agents/skills/bookkeeping/SKILL.md`, and runs in two modes: post-merge (given a merged PR) and a standalone no-argument drift sweep - both always check the whole repo; the argument only picks the starting point.
