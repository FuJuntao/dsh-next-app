...
test("serves the app shell and the home composer in a real browser", async ({ page }) => {
  ...
  await expect(async () => {
    // / renders the home page: the shell and the composer as the primary
    // element (story #117: the shared composer replaces the home placeholder;
    // the /sessions route is gone, the list lives in the side nav).
