...
test.describe("the basic-auth fence", () => {
  ...
  test("valid credentials pass the fence; wrong password and unknown user get 401", async () => {
    ...
    // The authenticated root renders the home page; assert a stable page
    // marker rather than chrome text that later stories may move.
    expect(await ok.text()).toContain("<title>Home</title>");
