...
## 2. Gather state
Read the GitHub issues live (story issues and their PR links, open and recently closed; plus ADR-0005-era parents, their `## Tasks` checklists, and task issues while any remain) at the chosen state - always across the whole repo, never only the starting point. Build the drift list - every place a record disagrees with a fact:
...
- ADR-0005-era legacy (ADR-0013 keeps their old mechanics until they close): a task issue not closed by its PR's closing syntax; a parent's checklist line disagreeing with its task issues' actual state; a parent whose sub-issues are all complete but which is still open.
...
- ADR-0005-era legacy parents (until the last one closes): tick or untick `## Tasks` checklist lines to match their task issues' actual state, and close a parent whose checklists are complete - GitHub closes task issues automatically via PR closing syntax; parents need explicit closing.
