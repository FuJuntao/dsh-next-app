...
## 2. Gather context
Before reviewing, read: the PR (diff, body, commits, status checks), the story issue it closes or references (`Closes`/`Part of`; the story's content and acceptance criteria live in the issue body), ADRs relevant to the change, and whatever contributor conventions (AGENTS.md or equivalent) apply in the environment this runs in. Never invent constraints the context does not contain.
...
- **Spec** (always): the diff satisfies the story's acceptance criteria as carried in the story issue - or, for a slice PR (`Part of`), the criteria its body claims. Blockers trace to the acceptance criteria or the story.
- **System design** (always): repo-wide constraints - ADR compliance, workspace boundaries, API shape. Blockers trace to the ADR or constraint violated; a PR that satisfies its slice yet violates an ADR fails here.
...
- **UI** (only when the diff touches UI code): the story issue's `## Design` packet - written by the `design` skill - carries this axis's criteria; blockers trace to it or, when the story predates the packet, to the acceptance criteria. A UI-touching diff whose story issue has no packet is itself a finding. Exercise the built app with the `agent-browser` skill against the PR's changes and the packet's states; if the app cannot be built or run in this environment, say plainly that the UI axis was reviewed statically only.
- **Security** (always): no secrets or leaked personal data; public-repo hygiene per ADR-0004.
- **Docs discipline** (always): doc changes ship together with the implying change, ADRs stay frozen (ADR-0004), issue records agree with the facts they record; volatile state is referred to its source of truth - a doc that restates what a lock file, directory, or tracker already tracks is a blocker.
- **Verification claims** (always): the PR body states what was run - re-run it or check its plausibility. A missing claim is a blocker (the `implement` contract requires one); a claim that does not reproduce is a blocker.
...
- **Request changes**: numbered findings, each tagged with its axis and traceable to its source (acceptance criteria, ADR, convention). Separate blocking from non-blocking plainly - no rubber stamp, no style noise. The skill ends here; re-review after fixes is a new invocation.
